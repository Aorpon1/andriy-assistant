 ...
 *
 *  ВАЖЛИВО ПРО ЖИВЛЕННЯ (СЕРВО MG996R!):
 *   - MG996R тягнуть 0.5..1А кожне (пік до 2.5А). ДВА серво = до ~2А!
 *   - Живити ОКРЕМИМ 5..6В з запасом струму МІНІМУМ 3А (BEC/UBEC або блок).
 *     Від піна 5V/3V3 плати ESP серво живити НЕ МОЖНА — просадка напруги,
 *     серво смикаються/стоять, плата перезавантажується.
 *   - GND цього живлення ОБОВ'ЯЗКОВО з'єднати з GND ESP8266 (спільна земля)!
 *   - Червоний серво -> +5..6В; коричневий -> спільний GND;
 *     сигнальний (помаранч.) -> GPIO5(D1)=PAN, GPIO4(D2)=TILT.
